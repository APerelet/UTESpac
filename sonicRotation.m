...
if isfield(sensorInfo,'u')
    ...
    for ii = 1:numSonics
        ...
        dirAvg = output.spdAndDir(:,strcmp(output.spdAndDirHeader,[num2str(sonHeight),'m Direction [deg]']));
        ...
        elseif strcmp(info.PF.globalCalculation,'global') && info.PF.recalculateGlobalCoefficients && exist('PFinfo','var') && ~isfield(PFinfo,sprintf('cm_%g',sonHeight*100))
            ...
            % find wind flag
            windFlagName = strcat(num2str(sonHeight), 'm Dir_Flag_'); % name of wind flag column
        ...
        output.rotatedSonicHeader{1,3*ii-2} = strcat(num2str(sonHeight), 'm Streamwise_u [m s^-1]');
        output.rotatedSonicHeader{1,3*ii-1} = strcat(num2str(sonHeight), 'm Spanwise_v [m s^-1]');
        output.rotatedSonicHeader{1,3*ii} = strcat(num2str(sonHeight), 'm Vertical_w [m s^-1]');
